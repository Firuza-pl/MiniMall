...
namespace Yasamall.Controllers
{
    public class ShopController : Controller
    {
        ...
        public async Task<IActionResult> Products(int? id)
        {
            ...
            ViewBag.Active = "Shop";
            ViewBag.ColorCount = _context.ProductColors.Where(c => c.ProductsId == product.Id).Count();
            ViewBag.SizeCount = _context.ProductSizes.Where(c => c.ProductsId == product.Id).Count();


            ProductsViewModel viewModel = new ProductsViewModel()
            {
                Product = product,
                AllProduct = _context.Products.Where(p => p.BrandsId == product.BrandsId && p.Id != id).Take(4),
        ...
        public IActionResult CreateProduct(int? id)
        {
            ...
            if (brand.IsOnePage) return View("Error");
        ...
        public async Task<IActionResult> CreateProduct(ProColors colors)
        {
            ...
            foreach (var p in colors.AllPhotos)
            {
                if (p != null)
                {
                    ...
                    {
                        string filename = await p.SaveAsync(_env.WebRootPath, "products");
